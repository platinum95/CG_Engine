...
private:
    ...
    std::unordered_map<ProjectionMapType, std::shared_ptr<CG_Data::Texture>> textureMaps;
...
public:
    CausticMapping( uint16_t _fbWidth, uint16_t _fbHeight, glm::vec3 _dir, float _distance, std::filesystem::path splatterPath );

    // Functions for receiver-object render passes
    std::shared_ptr<RenderPass> addReceiverPass( uint16_t _modelMatrixIdx = -1 );
    ...
    Camera mappingCamera;
    enum TextureType {
        ReceiverWorldspaceTexture,
        CausticSplatterTexture,
        CausticDepthTexture
    };
...
private:
    ...
    std::unordered_map< TextureType, std::shared_ptr<CG_Data::Texture>> textureMaps;
